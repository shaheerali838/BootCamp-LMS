...
function MyAttendance() {
  const { user } = useAuth();
  const { rawAttendance = [], myAttendanceStats, fetchAttendance, getStudentAttendance } = useAttendance();
  ...
  const presentCount = myAttendanceStats?.presentDays ?? history.filter((item) => item.status === "Present").length;
  const lateCount = myAttendanceStats?.lateDays ?? history.filter((item) => item.status === "Late").length;
  const leaveCount = myAttendanceStats?.leaveDays ?? history.filter((item) => item.status === "Leave").length;
  const absentCount = myAttendanceStats?.absentDays ?? history.filter((item) => item.status === "Absent").length;
    ...
    <div className="p-5 space-y-6">
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-2">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">My Attendance Portal</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {attendancePercentage >= 85 ? "Qualifying Status: Excellent" : "Target: 80% Minimum"}
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Late / Leave</p>
            <FiClock className="text-amber-500" size={20} />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mt-2">{lateCount + leaveCount}</h2>
          <p className="text-xs text-amber-600 mt-1">{lateCount} Late • {leaveCount} Leave</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-xs">
        <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 font-medium">Total Sessions: {history.length}</span>
        </div>
        ...
        <div className="divide-y divide-gray-100">
          ...
          {history.map((item, idx) => (
            <div key={item._id || idx} className="grid grid-cols-4 px-5 py-3.5 items-center hover:bg-gray-50/50 text-xs">
              <span className="font-semibold text-gray-800">{item.date}</span>
              <span className="text-gray-600 font-medium">{item.checkInTime || item.time || "--:--"}</span>
              <span className="text-gray-500">{item.checkOutTime || "--:--"}</span>
          ...
          {history.length === 0 && (
            <div className="py-12 text-center text-gray-400 space-y-1">
              <FiAlertCircle size={24} className="mx-auto opacity-50 text-gray-300" />
              <p className="text-xs font-medium">No attendance records logged yet.</p>
            </div>
          )}
        </div>
      </div>
